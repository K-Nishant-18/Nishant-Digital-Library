...
<button className="primary flex items-center gap-2 px-4 py-2 rounded-lg shadow-md hover:shadow-lg transition-shadow" onClick={onAddNote}>
  <Plus size={16} /> New Entry
</button>
      </div>

      {/* Filter and Search Bar */}
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-slate-900/60 p-4 rounded-xl border border-white/5 shadow-sm">
  ...
  />
        ...
        <div className="flex flex-wrap items-center gap-1.5 w-full md:w-auto">
          {['all', 'quote', 'note', 'reflection', 'highlight'].map(type => (
            <button
              key={type}
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg capitalize transition-colors ${activeType === type
                ...
                return (
                  <div key={note.id} className="panel space-y-3 relative group hover:border-amber-500/30 transition-all p-4 rounded-xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-slate-950/80 shadow-md">
                    <div className="flex items-center justify-between text-xs">
                      <span className={`font-semibold capitalize px-2 py-0.5 rounded text-[10px] ${note.type === 'quote' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' :
                    ...
                    <p className={`text-sm text-slate-200 leading-relaxed font-sans ${note.type === 'quote' ? 'italic font-serif text-amber-100/90 text-base bg-amber-500/10 p-3 rounded border border-amber-500/20' : ''}`}>
